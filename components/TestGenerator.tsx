...
const ExamCard: React.FC<{ icon: React.ReactNode; title: string; onClick: () => void }> = ({
  icon,
  title,
  onClick,
}) => (
  ...
  >
    <div className="flex items-center justify-center h-16 w-16 mx-auto mb-4 transition-transform duration-300 group-hover:scale-110">
      {icon}
    </div>
...
const Onboarding: React.FC = () => {
  ...
  // ⏱ 20s safety timeout – loading bahut der tak na atka rahe
  useEffect(() => {
    if (!loading) return;
    const id = window.setTimeout(() => {
      setLoading(false);
      setError('It is taking longer than expected. Please try again.');
    }, 20000);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLoading(false);
      navigate('/test', {
        state: { testType, numQuestions: 25, isDiagnostic: true },
      });
    } catch (err: any) {
      console.error('Failed to set goal / start test', err);
      const msg =
        err?.data?.message ||
        err?.message ||
  ...
  const renderStep1 = () => (
    <div className="w-full max-w-4xl text-center animate-fade-in">
      <h2 className="text-3xl font-bold text-slate-800 mb-2 flex items-center justify-center gap-3">
        <span className="bg-brand-blue text-white rounded-full w-10 h-10 inline-flex items-center justify-center font-bold text-xl">
          1
        </span>
        Select Your Exam
      </h2>
  ...
  const renderStep2 = () => (
    <div className="w-full max-w-lg text-center animate-fade-in">
      ...
      <div className="bg-white p-10 mt-10 rounded-2xl shadow-xl border border-slate-200">
        {error && (
          <p className="mb-4 rounded-lg bg-red-100 text-red-700 text-sm font-semibold px-4 py-2 text-left">
            {error}
          </p>
        )}
        ...
        >
          <div>
            <label
              htmlFor="goalScore"
              className="block text-sm font-medium text-slate-600 mb-2 text-left"
            >
              Target Score ({currentConfig.min}-{currentConfig.max})
            </label>
            <input
              type="number"
              id="goalScore"
          ...
          <div>
            ...
            <input
              ...
              onBlur={() => {
                if (!examDate) setDateInputType('text');
              }}
              id="examDate"
              value={examDate}
            ...
            <p className="text-left text-xs text-slate-500 mt-2">
              Leave blank for a default 3-month plan.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 pt-4">
            <button
              type="button"
              onClick={() => setStep(1)}
              className="text-slate-600 font-bold py-3 px-5 rounded-lg transition-colors hover:bg-slate-100"
              disabled={loading}
            >
              &larr; Back
            </button>
            <button
              type="submit"
              className="flex-1 bg-brand-blue text-white font-bold py-3 px-6 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50"
              disabled={loading}
            >
              {loading ? 'Generating...' : 'Set Goal & Continue'}
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md m-4 text-slate-800">
        <div className="p-8 text-center">
          ...
          <p className="text-slate-500 mb-8">
            Please review your selections before we generate your personalized plan.
          </p>
          <div className="space-y-4 text-left bg-slate-50 p-4 rounded-lg border border-slate-200">
            <div className="flex justify-between items-center">
              <span className="font-medium text-slate-500">Exam:</span>
              <span className="font-bold text-brand-blue">{examType}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="font-medium text-slate-500">Target Score:</span>
              <span className="font-bold">{goalScore}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="font-medium text-slate-500">Exam Date:</span>
              <span className="font-bold">
                {examDate
                  ? new Date(examDate + 'T00:00:00').toLocaleDateString()
                  : '3-Month Plan (Default)'}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-4 mt-8">
            <button
              ...
              className="w-full bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-3 px-6 rounded-lg transition-colors"
              disabled={loading}
            >
              Go Back
            </button>
            <button
              ...
              disabled={loading}
              className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-6 rounded-lg transition-colors flex items-center justify-center disabled:opacity-50"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 bottom-0 bg-white">
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-white via-blue-50 to-orange-50 z-0" />
      </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 z-10">
        <div className="text-center w-full max-w-4xl mb-12">
          ...
          <p className="text-slate-500 mt-3 max-w-xl mx-auto">
            Follow these simple steps to get a personalized plan.
          </p>
        </div>
