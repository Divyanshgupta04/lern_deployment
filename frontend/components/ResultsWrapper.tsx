...
const ResultsWrapper: React.FC = () => {
    ...
    const mutationInitiatedFor = useRef<string | null>(null);
    ...
    useEffect(() => {
        if (result?.isDiagnostic && user && result._id !== mutationInitiatedFor.current) {
            mutationInitiatedFor.current = result._id;
