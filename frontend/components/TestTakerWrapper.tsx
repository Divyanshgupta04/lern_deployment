...
const TestTakerWrapper: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      if (!testType) {
        throw new Error('Test parameters not provided.');
      }
  ...
  if (isError) {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-slate-600 mt-2">{err.message}</p>
        <button
          onClick={() => navigate('/dashboard')}
          className="mt-6 bg-brand-blue text-white font-bold py-2 px-5 rounded-lg"
        >
          Back to Dashboard
        </button>
      </div>
